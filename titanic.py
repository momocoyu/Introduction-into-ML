...
class RandomClassifier(Classifier) :
    ...
    def fit(self, X, y) :
        ...
        self.probabilities_ = sum(y)/len(y);
    ...
    def predict(self, X, seed=1234) :
        ...
        y = np.random.choice(2, a, p=[1 - self.probabilities_, self.probabilities_])
...
def main():
    ...
    for k in range(1,50,2):
        clf = KNeighborsClassifier(n_neighbors=k) # create MajorityVote classifier, which includes all model parameters
        validation_score=np.mean(cross_val_score(clf,X,y,cv=10))
        V.append(validation_score)
        xlabel.append(k)
    plt.plot(xlabel,V)
    plt.xlabel('k')
    ...
    for tt in range(1,20):
        clf = DecisionTreeClassifier(criterion='entropy',max_depth=tt) # create MajorityVote classifier, which includes all model parameters
        train_error,test_error = error(clf,X,y)
        tr.append(train_error)
        te.append(test_error)
        T.append(tt)
    plt.plot(T,tr,label="Decision Tree training error")
    plt.plot(T,te,label="Decision Tree test error")
    plt.xlabel('depth')
    plt.ylabel('error')
    ...
    amount=[]
    for i in range(1, 10):
        a, b = error(clf, X, y, test_size=i*0.1)
        dt_best_train_error.append(a)
        dt_best_test_error.append(b)
        c,d=error(clf_k,X,y,test_size=i*0.1)
        k_train_error.append(c)
        k_test_error.append(d)
        train_size=1-i*0.1
        amount.append(train_size) 


    plt.plot(amount,dt_best_train_error, label="Decision Tree training error(depth=6)")
    plt.plot(amount,dt_best_test_error, label="Decision Tree testing error(depth=6)")
    plt.plot(amount,k_train_error, label="KNN training error(k=7)")
    plt.plot(amount,k_test_error, label="KNN testing error(k=7)")
